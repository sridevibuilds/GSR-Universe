// Presentation Layer - Parent View Progress Card Screen
...
import '../../data/models/progress_card_model.dart';
...
class ParentProgressCardPage extends StatelessWidget {
  const ParentProgressCardPage({super.key});
  ...
  Future<void> _handlePdfAction(BuildContext context, String? fileName, String? filePath, {bool isDownload = false}) async {
    final cleanName = (fileName != null && fileName.trim().isNotEmpty)
        ? fileName.trim()
        : (filePath != null && filePath.trim().isNotEmpty ? filePath.trim().split('/').last : 'Progress_Card.pdf');
    ...
    if (bytes != null && bytes.isNotEmpty) {
      ...
      if (context.mounted) {
        AppNotifications.showError(context, "Unable to load progress card PDF.");
      }
    }
  }

  void _viewProgressCard(BuildContext context, String? fileName, String? filePath) {
    final cleanName = fileName ?? (filePath != null ? filePath.split('/').last : 'Progress_Card.pdf');
          ...
          content: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.badge_outlined, color: AppColors.parentPrimary),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        filePath ?? "Uploaded class progress card",
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ParentCubit, ParentState>(
      builder: (context, state) {
        final rawCards = state.progressCards;
        final List<ProgressCardModel> progressCards = rawCards.map((item) {
          return ProgressCardModel.fromJson(item);
        }).toList();
        ...
        return Scaffold(
          ...
          body: SafeArea(
            child: progressCards.isEmpty
                ? const Center(
                    child: EmptyStateWidget(
                      title: "No Progress Card Available",
                      message: "Official progress report cards uploaded by faculty will list here.",
                    ),
                  )
                : ListView.separated(
                    padding: const EdgeInsets.all(16),
                    itemCount: progressCards.length,
                    separatorBuilder: (context, index) => const SizedBox(height: 16),
                    itemBuilder: (context, index) {
                      final card = progressCards[index];
                      final String fileName = card.fileName;
                      final String? filePath = card.filePath;
                      final String academicYear = card.academicYear;
                      final String uploadedBy = card.uploadedByName;
                      final String remarks = card.remarks;

                      String uploadedDateStr = 'N/A';
                      if (card.uploadedAt.isNotEmpty) {
                        try {
                          uploadedDateStr = DateFormat('dd-MMM-yyyy').format(
                            DateTime.parse(card.uploadedAt),
                          );
                        } catch (_) {
                          uploadedDateStr = card.uploadedAt;
                        }
                      }
                      ...
                      return Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              "Uploaded by $uploadedBy on $uploadedDateStr",
                              style: GoogleFonts.inter(fontSize: 12, color: AppColors.textLight),
                            ),
                              ...
                              children: [
                                Expanded(
                                  child: OutlinedButton.icon(
                                    ...
                                    label: Text("View Progress Card", style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.bold)),
                                    onPressed: () => _viewProgressCard(context, fileName, filePath),
                                  ),
                                ),
                                const SizedBox(width: 8),
                                Expanded(
