// Presentation Layer - Parent View Assignments Screen (Three Tabs - Fixed Local/Network File Viewer & Interactive Full Screen Preview)
import 'dart:io';
...
class _ParentAssignmentsPageState extends State<ParentAssignmentsPage> with SingleTickerProviderStateMixin {
  ...
  void _showSubmitDialog(BuildContext context, int assignmentId, String title, {bool isReplace = false}) {
    String? filePath;
    String? fileName;
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Text(
                        isReplace ? "Replace Submission" : "Submit Assignment",
                  ...
                  FileUploader(
                    label: "Select Assignment File / PDF Document",
                    onFileSelected: (file) {
                      setModalState(() {
                        if (file != null) {
                          filePath = file.path;
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: filePath == null
                          ...
                          : () {
                              Navigator.pop(ctx);
                              context.read<ParentCubit>().submitAssignment(assignmentId, fileName!, filePath!);
                              AppNotifications.showSuccess(
                                context,
                                isReplace ? "Submission replaced successfully." : "Assignment submitted successfully.",
                              );
                            },
                      child: Text(
                        isReplace ? "Confirm & Replace" : "Submit Assignment",
  ...
  void _confirmDeleteSubmission(BuildContext context, int assignmentId) {
    ConfirmationDialog.show(
      context,
      title: "Delete Submission",
      content: "Are you sure you want to delete your submitted assignment before the deadline?",
      confirmText: "Delete",
      confirmColor: AppColors.danger,
      onConfirm: () {
        context.read<ParentCubit>().deleteAssignmentSubmission(assignmentId);
        AppNotifications.showSuccess(context, "Submission removed successfully.");
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ParentCubit, ParentState>(
      builder: (context, state) {
        final rawAssignments = state.assignments;
        final allAssignments = rawAssignments.map((item) => AssignmentModel.fromJson(item)).toList();

        // Filter assignments by subject
        final filteredAssignments = allAssignments.where((item) {
          if (_selectedSubject == 'All Subjects') return true;
          return item.subject.toLowerCase().contains(_selectedSubject.toLowerCase());
        }).toList();

        // History items
        final submittedHistory = allAssignments.where((item) => item.isSubmitted).toList();

        // Marks items
        final marksList = state.assignmentMarks;
        ...
        return Scaffold(
          ...
          body: SafeArea(
            child: TabBarView(
              ...
              children: [
                // TAB 1: ASSIGNMENTS
                _buildAssignmentsTab(context, filteredAssignments),

                // TAB 2: HISTORY
                _buildHistoryTab(context, submittedHistory),

                // TAB 3: MARKS
                _buildMarksTab(context, marksList),
              ],
            ),
          ),
        );
  ...
  Widget _buildAssignmentsTab(BuildContext context, List<AssignmentModel> assignmentsList) {
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Container(
            ...
            child: Row(
              children: [
                const Icon(Icons.filter_list, color: AppColors.parentPrimary, size: 20),
          ...
          child: assignmentsList.isEmpty
              ? const Center(
                  child: EmptyStateWidget(
                    ...
                    message: "There are no assignments published for the selected subject.",
                  ),
                )
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    String publishedDateStr = 'N/A';
                    if (item.publishedDate != null && item.publishedDate!.isNotEmpty) {
                      try {
                        publishedDateStr = DateFormat('dd-MMM-yyyy').format(DateTime.parse(item.publishedDate!));
                      } catch (_) {
                        publishedDateStr = item.publishedDate!;
                      }
                    }
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              const Icon(Icons.event, size: 14, color: AppColors.textLight),
                              ...
                              const Spacer(),
                              const Icon(Icons.alarm, size: 14, color: AppColors.danger),
                              const SizedBox(width: 4),
                              Text(
                                "Due: $dueDateStr",
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            item.attachmentName ?? 'Faculty_Assignment.pdf',
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            item.submissionFile ?? 'Submission.pdf',
                                            style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.textDark),
                                            overflow: TextOverflow.ellipsis,
                                          ),
                                        ],
                            ...
                            children: [
                              ...
                              Expanded(
                                ...
                                child: ElevatedButton.icon(
                                  ...
                                  label: Text(
                                    item.isSubmitted ? "Replace Submission" : "Submit Assignment",
  ...
  Widget _buildHistoryTab(BuildContext context, List<AssignmentModel> historyList) {
    if (historyList.isEmpty) {
      return const Center(
        child: EmptyStateWidget(
          title: "No Submission History",
          message: "Previously submitted assignment tasks will be archived here.",
        ),
      );
    }
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final String fileName = item.submissionFile ?? item.attachmentName ?? 'Submission.pdf';
  ...
  Widget _buildMarksTab(BuildContext context, List<Map<String, dynamic>> marksList) {
    if (marksList.isEmpty) {
      return const Center(
        child: EmptyStateWidget(
          title: "No Marks Evaluated Yet",
          message: "Assignment grades and marks published by your subject teachers will display here.",
        ),
      );
    }
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final String title = item['title'] ?? 'Assignment Task';
        final String subject = item['subject_name'] ?? item['subject'] ?? 'General';
        final double obtained = (item['obtained_marks'] as num? ?? 0.0).toDouble();
        final double maxMarks = (item['max_marks'] as num? ?? 20.0).toDouble();
          ...
          child: Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  "${obtained.toStringAsFixed(1)} / ${maxMarks.toStringAsFixed(0)}",
                  style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.parentPrimary),
                ),
              ),
            ],
          ),
